...
def user_auth_checker(event):
    if event.is_private:
        ...
        if event.message.sender.id in sudo_users or event.message.sender.id in allowed_chats or event.message.sender.id == owner_id or event.message.sender.id == auth_chat:
            return True
    return False
